...
const UNITS = [
  ...
  { id: "tsp", label: "Teaspoon (US tsp)", factor: 0.00492892159 },
  { id: "tbsp", label: "Tablespoon (US tbsp)", factor: 0.01478676478 },
  { id: "floz", label: "Fluid ounce (US fl oz)", factor: 0.0295735296 },
  { id: "cup", label: "Cup (US)", factor: 0.2365882365 },
  { id: "pint", label: "Pint (US pt)", factor: 0.4731764735 },
  { id: "qt", label: "Quart (US qt)", factor: 0.946352947 },
  { id: "gal", label: "Gallon (US gal)", factor: 3.785411784 },
  { id: "cuin", label: "Cubic inch (in³)", factor: 0.016387064 },
  { id: "cuft", label: "Cubic foot (ft³)", factor: 28.316846592 },
];
...
function convert(value: number, fromId: string, toId: string): number {
  const liters = value * getFactor(fromId);
  return liters / getFactor(toId);
}

type Props = { initialValue?: string; initialFrom?: string; initialTo?: string };

export function VolumeConverter({ initialValue = "1", initialFrom = "L", initialTo = "gal" }: Props) {
