...
const UNITS = [
  ...
  { id: "ftlb", label: "Foot-pound (ft·lb)", factor: 1.355818 },
];
...
function convert(value: number, fromId: string, toId: string): number {
  const joules = value * getFactor(fromId);
  return joules / getFactor(toId);
}

type Props = { initialValue?: string; initialFrom?: string; initialTo?: string };

export function EnergyConverter({ initialValue = "1", initialFrom = "kJ", initialTo = "kcal" }: Props) {
