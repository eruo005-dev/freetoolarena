...
const UNITS = [
  ...
  { id: "stone", label: "Stone (st)", factor: 6350.29318 },
  { id: "tonne", label: "Tonne (t)", factor: 1000000 },
];
...
function convert(value: number, fromId: string, toId: string): number {
  const grams = value * getFactor(fromId);
  return grams / getFactor(toId);
}

type Props = { initialValue?: string; initialFrom?: string; initialTo?: string };

export function WeightConverter({ initialValue = "1", initialFrom = "kg", initialTo = "lb" }: Props) {
